...
extension _TimeOfDayX on MeetingTimeOfDay {
  ...
  IconData get icon => switch (this) {
        MeetingTimeOfDay.morning   => Icons.wb_sunny_outlined, // sunrise
        MeetingTimeOfDay.afternoon => Icons.cloud_outlined,    // partly cloudy
        MeetingTimeOfDay.evening   => Icons.brightness_5,      // dimmer sun
        MeetingTimeOfDay.night     => Icons.brightness_3,      // crescent moon
      };
...
MeetingTimeOfDay _currentTimeOfDay() {
...
int _todayWeekday() => DateTime.now().weekday % 7;
  ...
  List<Meeting> apply(List<Meeting> meetings) {
    return meetings.where((m) {
      ...
      if (typeCodes.isNotEmpty) {
        final parsed = _parseCodes(m.typeCodes);
...
/// Default filter: today's day of week + current time of day.
MeetingFilter _defaultFilter() => MeetingFilter(
      weekday: _todayWeekday(),
...
// ─────────────────────────────────────────────────────────────────────────────
// Meeting stream providers
// ─────────────────────────────────────────────────────────────────────────────

final _allMeetingsProvider = StreamProvider<List<Meeting>>((ref) {
  return ref.watch(meetingsRepositoryProvider).watchAll();
});

final _bookmarkedMeetingsProvider = StreamProvider<List<Meeting>>((ref) {
  return ref.watch(meetingsRepositoryProvider).watchBookmarked();
});

final _attendanceProvider = StreamProvider<List<AttendanceLog>>((ref) {
  return ref.watch(meetingsRepositoryProvider).watchAttendance();
});

final filteredMeetingsProvider = Provider<List<Meeting>>((ref) {
  final all = ref.watch(_allMeetingsProvider).valueOrNull ?? [];
...
class _FinderTab extends HookConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final allAsync = ref.watch(_allMeetingsProvider);
...
class _MeetingCard extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final types = _parseCodes(meeting.typeCodes);
...
class _MyMeetingsTab extends ConsumerWidget {
  ...
  @override
  Widget build(BuildContext context, WidgetRef ref) {
    final bookmarkedAsync = ref.watch(_bookmarkedMeetingsProvider);
...
List<String> _parseCodes(String json) {
  try {
